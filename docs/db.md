...
| **`enrichCache`**      | `get`, `upsert` — Vue TTL/orchestration: [`hubRest.ts`](../frontend/src/lib/hubRest.ts) `enrichCacheGet` / `enrichCacheSet` / `enrichCacheInvalidate` / `withEnrichCache` |
| **`userProfiles`**     | `get`                                                                                                                                                                     |

Internal `_req` throws on non-OK responses with a short error body for debugging.

**Vue:** [`hubRest.ts`](../frontend/src/lib/hubRest.ts) mirrors these tables for the SPA (Phase 1.5: intelligence, merge, per-slug relations, profiles; Phase 1.6: remaining `companies` / `contacts` / `audiences` / `relations.upsert` helpers).
