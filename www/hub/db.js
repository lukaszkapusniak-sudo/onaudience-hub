...
import { SB_URL } from './config.js?v=20260410d20';
import { authHdr } from './utils.js?v=20260410d20';
...
export const contacts = {
  ...
  byCompany: (slug, name) => {
    ...
    return Promise.all([
      ...
      const seen = new Set();
      return [...(Array.isArray(byId)?byId:[]), ...(Array.isArray(byName)?byName:[])]
        .filter(c => { const k=c.id||c.email; if(seen.has(k))return false; seen.add(k); return true; });
    });
  },
...
export const relations = {
  ...
  byCompany:  (slug) => Promise.all([
    ...
    const seen=new Set();
    return [...(Array.isArray(from)?from:[]),...(Array.isArray(to)?to:[])]
      .filter(r=>{ const k=`${r.from_company}|${r.to_company}|${r.relation_type}`; if(seen.has(k))return false; seen.add(k); return true; });
  }),
