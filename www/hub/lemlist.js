...
import { SB_URL, LEMLIST_PROXY } from './config.js?v=__OA_ASSET_VERSION__';
import S from './state.js?v=__OA_ASSET_VERSION__';
import { esc, _slug, relTime, authHdr } from './utils.js?v=__OA_ASSET_VERSION__';
import {
  lemlistFetch,
  lemlistCampaigns,
  lemlistAddLead,
...
export async function openLemlistModal(contacts) {
  ...
  if (!modal) {
    initLemlistModal();
    modal = document.getElementById('llModal');
  }
  ...
  if (!status || !sel || !preview || !btn) {
    console.error('[Lemlist] modal DOM not ready');
    return;
  }
  try {
    ...
    if (!campaigns.length) {
      status.textContent = 'No campaigns found in lemlist. Create one first.';
      return;
    }
...
export async function lemlistPush() {
  ...
  for (const ct of _llContacts) {
    try {
      await lemlistAddLead(campaignId, ct);
      ok++;
    } catch (e) {
      fail++;
      clog('info', 'lemlist skip ' + esc(ct.email) + ': ' + e.message);
    }
  }
  const ids = _llContacts.map((c) => c.id).filter(Boolean);
  if (ids.length) {
    try {
      await lemlistWriteBack(ids, campaignId, campaignName);
    } catch (e) {
      clog('info', 'lemlist writeback error: ' + e.message);
    }
  }
  clog('db', '📤 lemlist: ' + ok + '/' + _llContacts.length + ' pushed → ' + campaignName);
  prev.textContent =
    '✓ ' + ok + ' pushed' + (fail ? ', ' + fail + ' skipped' : '') + ' → ' + campaignName;
...
export async function audPushLemlist(audId, campaignId = null) {
  ...
  if (!contacts.length) {
    alert('No contacts with email found for this audience.');
    return;
  }
  if (campaignId) {
    ...
    clog(
      'db',
      '📤 Pushing <b>' + contacts.length + '</b> contacts to <b>' + esc(campaignName) + '</b>…',
    );
    let ok = 0,
      fail = 0;
    for (const ct of contacts) {
      try {
        ...
        fail++;
        clog('info', 'skip ' + esc(ct.email || '') + ': ' + e.message);
      }
    }
...
export async function refreshLemlistCampaigns() {
  ...
  if (!panel) return;
  panel.innerHTML =
    '<div style="padding:20px 16px;font-size:11px;font-family:\'IBM Plex Mono\',monospace;color:var(--t3)">Loading campaigns\u2026</div>';
  try {
    ...
    panel.innerHTML =
      '<div style="padding:20px 16px;font-size:11px;font-family:\'IBM Plex Mono\',monospace;color:var(--cr)">\u26a0 ' +
      esc(e.message) +
      '</div>';
  }
}
...
export function renderLemlistPanel() {
  ...
  const keyHint = connected
    ? '● ' + localStorage.getItem('oaLemlistKey').slice(0, 8) + '…'
    : 'not connected';
  const syncAgo = _llLastSync
    ? ' · synced ' + Math.round((Date.now() - _llLastSync) / 60000) + 'm ago'
    : '';
  panel.innerHTML = `
  <div class="ll-header">
...
export async function selectLemlistCampaign(campaignId) {
  ...
  center.innerHTML =
    '<div style="padding:24px;font-size:11px;color:var(--t3);font-family:\'IBM Plex Mono\',monospace">Loading leads\u2026</div>';
  try {
...
function _renderCampaignDetail() {
  ...
  const filtered = _llLeadSearch
    ? leads.filter((l) =>
        ((l.email || '') + (l.firstName || '') + (l.lastName || '') + (l.companyName || ''))
          .toLowerCase()
          .includes(_llLeadSearch.toLowerCase()),
      )
    : leads;
  const audOptions = (S.audiences || [])
    ...
    .map(
      (a) =>
        `<option value="${esc(a.id)}">${esc(a.name)} (${(a.company_ids || []).length} co)</option>`,
    )
    .join('');
  center.style.display = 'flex';
  ...
  center.innerHTML = `<div class="ll-detail">
    ...
    <div class="ll-detail-stats">
      ...
      ${(() => {
        ...
        const bounced = leads.filter(
          (x) => x.status === 'bounced' || x.status === 'unsubscribed',
        ).length;
        if (!sent) return '';
        return (
          '<span title="Sent">📤 ' +
          ...
          '</span>' +
          (opened
            ? '<span title="Open rate" style="color:var(--g)">👁 ' +
              Math.round((opened / sent) * 100) +
              '%</span>'
            : '') +
          (replied
            ? '<span title="Reply rate" style="color:var(--pc)">💬 ' +
              Math.round((replied / sent) * 100) +
              '%</span>'
            : '') +
          (clicked
            ? '<span title="Click rate" style="color:var(--prc)">🖱 ' +
...
function _renderLeadRow(l, campaignId) {
  ...
  const acts =
    ...
    l.status === 'replied' ||
    l.status === 'interested'
      ? '<span title="Opened" style="color:var(--g)">👁</span>'
      : '') +
    (l.clickedAt || l.status === 'clicked'
      ? '<span title="Clicked" style="color:var(--prc)">🖱</span>'
      : '') +
    (l.repliedAt || l.status === 'replied' || l.status === 'interested'
      ? '<span title="Replied" style="color:var(--pc)">💬</span>'
      : '');
  ...
  const nameHtml = sbCt
    ...
    : name;
  // Company link
  const coSlug = (l.companyName || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
  const sbCo = S.companies?.find(
    (c) => c.id === coSlug || (c.name || '').toLowerCase() === (l.companyName || '').toLowerCase(),
  );
  const coHtml = sbCo
  ...
  return `<tr>
    ...
    <td><button class="btn sm" style="color:var(--cr)" title="Unsubscribe"
      onclick="llUnsubLead('${esc(campaignId)}','${esc(l.email || '')}')">&#10005;</button>${l.email ? `<button class="btn sm" style="font-size:7px;padding:0 5px;opacity:.75" title="Enrich via Vibe Prospecting (1 credit)" onclick="window.vibeEnrichLead&&window.vibeEnrichLead('${esc(l.email || '')}','${esc(((l.firstName || '') + ' ' + (l.lastName || '')).trim())}','${esc(l.companyName || '')}')">⚡</button>` : ''}
    </td>
  </tr>`;
}
...
export async function llPushFromAudience() {
  ...
  if (!audId || !_llSelCampaign) {
    alert('Select an audience first.');
    return;
  }
  const aud = (S.audiences || []).find((a) => a.id === audId);
  if (!aud) return;
  ...
  const contacts = (S.contacts || []).filter((c) => coIds.includes(c.company_id) && c.email);
  if (!contacts.length) {
    alert('No contacts with email in this audience.');
    return;
  }
  openLemlistModal(contacts);
}
  ...
  try {
    ...
    for (const l of allLeads) {
      ...
      try {
        ...
        clog('info', 'Lemlist contact save error: ' + e.message);
      }
    }
