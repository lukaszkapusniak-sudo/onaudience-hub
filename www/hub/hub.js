...
export function openCompany(c){
  ...
  const _coSlug=c.id||slug;const coCts=S.contacts.filter(ct=>ct.company_id===_coSlug||_slug(ct.company_name||'')===_coSlug||(ct.company_name||'').toLowerCase()===(c.name||"").toLowerCase()||_slug(ct.company_name||'')===_slug(c.name||''));
