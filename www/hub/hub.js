...
import { SB_URL, SB_KEY, TAG_RULES, MODEL_CREATIVE, MODEL_RESEARCH } from './config.js?v=20260410d23';
import S from './state.js?v=20260410d23';
import { classify, _slug, getCoTags, getAv, ini, tClass, tLabel, stars, esc, relTime, authHdr, safeUrl } from './utils.js?v=20260410d23';
import { renderStats, fetchGoogleNews, saveIntelligence, anthropicFetch, anthropicMcpFetch, researchFetch, refreshRelationsCache, saveContact, lemlistFetch, lemlistCampaigns, lemlistAddLead, lemlistWriteBack } from './api.js?v=20260410d23';
import { resolveAlias } from './merge.js?v=20260410d23';
import { companies as dbCompanies, contacts as dbContacts, relations as dbRelations, intelligence as dbIntel } from './db.js?v=20260410d23';
...
export async function runAI(){
  ...
  try{
    ...
    const poolMsg=data.pool_size<data.total?` (screened ${data.pool_size}/${data.total})`:'';
    ...
    clog('ai',`✓ AI search: <b>${names.length}</b> match${names.length!==1?'es':''} for "<b>${esc(q.slice(0,40))}</b>"${poolMsg}`);
    renderList();
  }catch(e){
    dot.className='ai-dot err';
    const msg=e.message||'Unknown error';
    const friendly=msg.includes('key')||msg.includes('401')?'Invalid API key — check 🔑 in nav'
      :msg.includes('429')||msg.includes('Rate')?'Rate limit — wait a moment and retry'
...
export function openCompany(c){
  ...
  const _audMemberChips=_memberAuds.map(a=>`<span class="sys-chip sys-chip-aud" onclick="audToggleCo('${esc(a.id)}','${esc(coId)}')" title="Remove from ${esc(a.name)}">📋 ${esc(a.name)} <span class="sys-chip-x">✕</span></span>`).join('');
  const _audAddOpts=_nonMemberAuds.length
    ?_nonMemberAuds.map(a=>`<div class="sys-dd-item" onclick="audToggleCo('${esc(a.id)}','${esc(coId)}');this.closest('.sys-dd').classList.remove('open')">📋 ${esc(a.name)} <span style="opacity:.4;font-size:8px">${(a.company_ids||[]).length} co</span></div>`).join('')
...
export { initLemlistModal, openLemlistModal, closeLemlistModal, lemlistPush,
  ...
  llSyncContacts, llSyncCompanies, llSetKey, llClearKey, llIsConnected } from './lemlist.js?v=20260410d23';

export { openDrawer, closeDrawer, openContactFull,
  drEmail, drLinkedIn, drGmail, drResearch } from './drawer.js?v=20260410d23';
...
export { tagCountsFor, countPool, matchTags, renderTagPanel, toggleTagPanel,
  ...
  setFilter, onSearch, setSort, renderList } from './list.js?v=20260410d23';
