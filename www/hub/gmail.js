...
import { GMAIL_CLIENT_ID } from './config.js?v=20260410d22';
import { esc, authHdr } from './utils.js?v=20260410d22';
import { SB_URL } from './config.js?v=20260410d22';
...
function _clearToken() {
  ['oaGmailToken', 'oaGmailExpiry', 'oaGmailEmail', 'oaGmailName'].forEach(function (k) {
...
function _getTokenClient() {
  ...
  _tokenClient = window.google.accounts.oauth2.initTokenClient({
    ...
    callback: function (res) {
      if (res.error) {
        if (_pendingReject) _pendingReject(new Error(res.error));
      } else {
        _saveToken(res.access_token, res.expires_in);
        if (_pendingResolve) _pendingResolve(res.access_token);
      }
      _pendingResolve = null;
      _pendingReject = null;
    },
...
async function _gFetch(path, params) {
  ...
  Object.entries(params).forEach(function (kv) {
    if (Array.isArray(kv[1])) {
      kv[1].forEach(function (v) {
        url.searchParams.append(kv[0], v);
      });
    } else {
  ...
  if (!res.ok) {
    ...
    throw new Error('Gmail API ' + res.status + ': ' + txt.slice(0, 100));
  }
  return res.json();
}
...
export async function gmailSearchCompany(companyName, domain) {
  ...
  var details = await Promise.allSettled(
    msgs.slice(0, 8).map(function (m) {
      return _gFetch('/messages/' + m.id, {
        format: 'metadata',
  ...
  details.forEach(function (r) {
    ...
    var hh = {};
    ((r.value.payload && r.value.payload.headers) || []).forEach(function (x) {
      hh[x.name] = x.value;
    });
    var from = hh['From'] || '',
      subject = hh['Subject'] || '(no subject)';
    var date = hh['Date']
      ? new Date(hh['Date']).toLocaleDateString('en-GB', {
          day: 'numeric',
          month: 'short',
          year: '2-digit',
        })
    ...
    threads.push({
      ...
      date: date,
      id: r.value.id,
      threadId: r.value.threadId || r.value.id,
    });
  ...
  return {
    threads: threads,
    contacts: Object.values(cmap),
    query: query,
    total: list.resultSizeEstimate || msgs.length,
  };
}
export function gmailSectionHTML(slug, companyName) {
  ...
  if (!gmailIsConnected())
    return (
      '<div style="padding:4px 0"><div style="font-size:10px;color:var(--t3);margin-bottom:10px">Connect Gmail to scan history with <b>' +
      n +
      '</b></div>' +
      '<button class="btn sm p" onclick="window.gmailConnectAndScan(\'' +
      s +
      "','" +
      n +
  ...
  return (
    ...
    '<span style="font:400 9px monospace;color:var(--t2);flex:1;overflow:hidden;text-overflow:ellipsis">' +
    esc(gmailGetStoredEmail()) +
    '</span>' +
    '<button class="btn sm" onclick="window.gmailDisconnectUI()">Disconnect</button></div>' +
    '<div style="display:flex;gap:6px;margin-bottom:4px">' +
    '<button class="btn sm p" onclick="window.gmailScanCompany(\'' +
    s +
    "','" +
    n +
    '\')">Scan Gmail</button>' +
    '<button class="btn sm" onclick="window.gmailEnrichContacts(\'' +
    s +
    "','" +
    n +
...
export function gmailDisconnectUI() {
  gmailDisconnect();
  var slug = window._currentEmailSlug,
    name = (window.currentCompany && window.currentCompany.name) || '';
  var body = document.getElementById('ib-email-body');
...
export function gmailRenderResults(threads, companyName) {
  ...
  if (!threads || !threads.length) {
    el.innerHTML =
      '<div style="font-size:9px;color:var(--t3);padding:4px 0">No emails found' +
      (companyName ? ' for <b>' + esc(companyName) + '</b>' : '') +
      '</div>';
    return;
  }
  el.innerHTML =
    '<div style="font:600 8px monospace;text-transform:uppercase;color:var(--t3);margin-bottom:6px">' +
    threads.length +
    ' email' +
    (threads.length !== 1 ? 's' : '') +
    '</div>' +
    threads
      .map(function (t) {
        return (
          '<div class="gmail-row" style="display:flex;gap:8px;padding:6px 0;border-bottom:1px solid var(--rule3)">' +
          ...
          (t.threadId || t.id || '') +
          '" target="_blank" style="font:500 11px monospace;color:var(--g);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-decoration:none;display:block" title="Open in Gmail">' +
          esc(t.subject) +
          ' ↗</a>' +
          '<div style="font:400 8px monospace;color:var(--t3);margin-top:2px;display:flex;gap:8px">' +
          '<span>' +
          esc((t.from || '').slice(0, 40)) +
          ((t.from || '').length > 40 ? '...' : '') +
          '</span>' +
...
export async function gmailScanCompany(slug, companyName) {
  ...
  if (!el) return;
  el.innerHTML =
    '<div style="font-size:9px;color:var(--t3);animation:pulse 1.4s infinite">Scanning Gmail...</div>';
  var co =
    window._oaState &&
    window._oaState.companies &&
    window._oaState.companies.find(function (c) {
      return (c.id || (window._slug && window._slug(c.name || ''))) === slug;
    });
  ...
  try {
    ...
    if (!threads.length) {
      el.innerHTML =
        '<div style="font-size:9px;color:var(--t3);padding:4px 0">No emails found for <b>' +
        esc(companyName) +
        '</b></div>';
      return;
    }
    ...
    if (contacts.length) {
      ...
      strip.innerHTML =
        ...
        ' contact' +
        (contacts.length > 1 ? 's' : '') +
        ' found &mdash; select to save:</div>' +
        contacts
          .map(function (c, i) {
            return (
              '<label style="display:flex;align-items:center;gap:7px;padding:4px 2px;cursor:pointer;border-bottom:1px solid var(--rule3)">' +
              '<input type="checkbox" checked data-i="' +
              i +
              '" style="accent-color:var(--g);cursor:pointer"/>' +
              '<span style="flex:1;font:500 10px monospace;color:var(--t1)">' +
              esc(c.name || c.email) +
              '</span>' +
              '<span style="font:400 9px monospace;color:var(--t4)">' +
              esc(c.email) +
              '</span>' +
              '</label>'
            );
    ...
    if (window.clog)
      window.clog('info', 'Gmail: ' + threads.length + ' emails, ' + contacts.length + ' contacts');
  } catch (e) {
    el.innerHTML =
      '<div style="font-size:9px;color:var(--prc)">Error: ' + esc(e.message) + '</div>';
...
export async function gmailSaveContacts() {
  ...
  for (var i = 0; i < contacts.length; i++) {
    try {
      ...
      // Generate id if missing — normalise unicode, then slug
      if (!ct.id) {
        ...
        var namePart = clean(ct.full_name || ct.email || '');
        var coPart = clean(ct.company_name || '');
        ct.id =
          (namePart + (coPart ? '-' + coPart : '')).slice(0, 80) ||
          (ct.email || '').replace('@', '--at--').replace(/\./g, '-');
      }
  ...
  if (strip)
    strip.innerHTML =
      '<div style="font-size:9px;color:var(--g)">Saved ' +
      saved +
      ' contact' +
      (saved !== 1 ? 's' : '') +
      ' to CRM</div>';
...
export async function gmailEnrichContacts(slug, companyName) {
  ...
  if (el)
    el.innerHTML =
      '<div style="font-size:9px;color:var(--t3);animation:pulse 1.4s infinite">Scanning Gmail for contacts...</div>';
  ...
  try {
    ...
    if (!msgs.length) {
      if (el)
        el.innerHTML =
          '<div style="font-size:9px;color:var(--t3)">No emails found for ' +
          esc(companyName) +
          '</div>';
      return;
    }
    ...
    var details = await Promise.allSettled(
      msgs.slice(0, 20).map(function (m) {
        return _gFetch('/messages/' + m.id, {
          format: 'metadata',
    ...
    var existingContacts =
      (window._oaState &&
        window._oaState.contacts &&
        window._oaState.contacts.filter(function (ct) {
          return (
            ct.company_id === slug ||
            (ct.company_name || '').toLowerCase() === companyName.toLowerCase()
          );
        })) ||
      [];

    var newContacts = contacts.filter(function (c) {
      return !existingContacts.some(function (ec) {
        return (ec.email || '').toLowerCase() === c.email;
      });
    });

    var enriched = existingContacts
      .filter(function (ec) {
        if (ec.email) return false; // already has email
        return contacts.some(function (c) {
          return (
            (ec.full_name || '').toLowerCase().includes(c.name.toLowerCase()) && c.name.length > 2
          );
        });
      })
      .map(function (ec) {
        var match = contacts.find(function (c) {
          return (ec.full_name || '').toLowerCase().includes(c.name.toLowerCase());
        });
    ...
    if (el) {
      el.innerHTML =
        ...
        ' emails scanned · ' +
        contacts.length +
        ' unique contacts found</div>' +
        (newContacts.length
          ? '<div style="font-size:9px;color:var(--g);margin-bottom:4px">' +
            newContacts.length +
            ' new contact' +
          ...
          : '') +
        (enriched.length
          ? '<div style="font-size:9px;color:var(--poc);margin-bottom:4px">' +
            enriched.length +
            ' existing contact' +
            (enriched.length > 1 ? 's' : '') +
            ' can get email added</div>'
          : '') +
        (!newContacts.length && !enriched.length
          ? '<div style="font-size:9px;color:var(--t3)">All contacts already in CRM with emails</div>'
          : '');
    }
    ...
    if (strip && (newContacts.length || enriched.length)) {
      ...
      strip.innerHTML =
        ...
        (enriched.length
          ? enriched
              .map(function (e) {
                var ct = existingContacts.find(function (ec) {
                  return ec.id === e.id;
                });
                return (
                  '<div style="display:flex;gap:6px;padding:3px 0;font-size:10px">' +
                  '<span style="color:var(--t1)">' +
                  esc((ct && ct.full_name) || e.id) +
                  '</span>' +
                  '<span style="color:var(--t4)">' +
                  esc(e.email) +
                  '</span>' +
              ...
              .join('')
          : '') +
        ((window._gmailEnriched = enriched),
        '<button class="btn sm p" onclick="window.gmailSaveAndEnrichContacts(window._gmailEnriched)" style="margin-top:6px">') +
        'Save ' +
        (newContacts.length + enriched.length) +
        ' update' +
        (newContacts.length + enriched.length > 1 ? 's' : '') +
        ' to CRM</button>';
    } else if (strip) {
      strip.style.display = 'none';
    }
...
export async function gmailSaveAndEnrichContacts(enriched) {
  ...
  for (var j = 0; j < enriched2.length; j++) {
    try {
      var res2 = await fetch(
        SB_URL + '/rest/v1/contacts?id=eq.' + encodeURIComponent(enriched2[j].id),
        {
          method: 'PATCH',
      ...
      if (res2.ok) {
        ...
        var ct =
          window._oaState &&
          window._oaState.contacts &&
          window._oaState.contacts.find(function (c) {
            return c.id === enriched2[j].id;
          });
  ...
  if (strip)
    strip.innerHTML =
      ...
      ' new + ' +
      patched +
      ' emails added to CRM</div>';
  window._gmailFoundContacts = [];
  if (window.clog)
    window.clog('db', 'Gmail enrich done: ' + saved + ' new, ' + patched + ' emails patched');
}
...
export function gmailShowSummarizePrompt(slug, companyName, threads) {
  ...
  companyName =
    companyName ||
    window._gmailLastName ||
    (window.currentCompany && window.currentCompany.name) ||
    '';
  ...
  if (!threads.length) {
    ...
    if (el2)
      el2.innerHTML =
        '<div style="font-size:9px;color:var(--prc)">Scan Gmail first to load emails before summarizing.</div>';
    return;
  }
  ...
  el.innerHTML =
    existing +
    '<div id="gmail-sum-confirm" style="margin-top:12px;padding:10px 12px;background:var(--surf2);border:1px solid var(--rule);border-radius:2px">' +
    ...
    ' email' +
    (threads.length > 1 ? 's' : '') +
    ' (subjects, dates, contacts) to generate a relationship summary.<br>' +
    '<span style="color:var(--t2)">Est. ' +
    est.total +
    ' tokens &middot; ' +
    cost +
    '</span><br>' +
...
export async function gmailRunSummarize() {
  ...
  if (confirm)
    confirm.innerHTML =
      '<div style="font-size:9px;color:var(--t3);animation:pulse 1.4s infinite">Summarizing with Claude...</div>';

  if (!threads.length) {
    if (confirm)
      confirm.innerHTML =
        '<div style="font-size:9px;color:var(--prc)">No email data to summarize.</div>';
    return;
  }
  ...
  if (!window.anthropicFetch && !window.getApiKey) {
    if (confirm)
      confirm.innerHTML =
        '<div style="font-size:9px;color:var(--prc)">Anthropic API key required — click the key icon in nav.</div>';
    return;
  }

  // Build prompt from thread metadata
  var emailList = threads
    .map(function (t, i) {
  ...
  var systemPrompt =
    'You are a B2B sales analyst for onAudience (EU first-party data company). Analyze these email metadata entries and summarize the relationship with the company. Be concise and specific. Format as JSON only.';
  ...
  try {
    ...
    if (confirm) {
      if (parsed) {
        var statusColor =
          { warm: 'var(--cc)', cold: 'var(--t3)', active: 'var(--g)', dormant: 'var(--prc)' }[
            parsed.relationship_status
          ] || 'var(--t2)';
        confirm.innerHTML =
          '<div style="font:600 9px monospace;text-transform:uppercase;letter-spacing:.06em;color:var(--t2);margin-bottom:8px;display:flex;align-items:center;gap:8px">' +
          'Relationship Summary' +
          '<span style="font-size:8px;color:var(--t4);font-weight:400;text-transform:none;margin-left:auto">' +
          inputUsed +
          '+' +
          outputUsed +
          ' tokens &middot; ' +
          ...
          _sumField(
            ...
            '<span style="color:' +
              ...
              esc(parsed.relationship_status || '?') +
              '</span>',
          ) +
          ...
          '</div>' +
          (parsed.main_topics && parsed.main_topics.length
            ? '<div style="font:400 9px monospace;color:var(--t3);margin-bottom:6px">Topics: ' +
              parsed.main_topics
                .map(function (t) {
                  return esc(t);
                })
            ...
            : '') +
          (parsed.summary
            ? '<div style="font:400 10px IBM Plex Sans,sans-serif;color:var(--t2);line-height:1.55;margin-bottom:6px;font-style:italic">' +
              esc(parsed.summary) +
              '</div>'
            : '') +
          (parsed.recommended_action
            ? '<div style="padding:6px 8px;background:var(--gb);border-left:2px solid var(--g);font:400 10px IBM Plex Sans,sans-serif;color:var(--t1)">' +
              '<span style="font:600 8px monospace;color:var(--g);text-transform:uppercase;letter-spacing:.05em">Recommended: </span>' +
              esc(parsed.recommended_action) +
              '</div>'
            : '') +
          '<div style="display:flex;gap:5px;margin-top:8px">' +
          ((window._gmailSlug = slug),
          (window._gmailParsed = parsed),
          '<button class="btn sm" onclick="window.gmailSaveRelationshipSummary(window._gmailSlug,window._gmailParsed)">Save to Intelligence</button>') +
          '<button class="btn sm" onclick="document.getElementById(\'gmail-sum-confirm\').remove()">Close</button>' +
          '</div>';
      } else {
        confirm.innerHTML =
          '<div style="font-size:9px;color:var(--t2);white-space:pre-wrap">' +
          esc(text.slice(0, 400)) +
          '</div>' +
    ...
    if (confirm)
      confirm.innerHTML =
        '<div style="font-size:9px;color:var(--prc)">Error: ' +
        esc(e.message) +
        '</div>' +
...
function _sumField(label, value) {
  return (
    '<div style="padding:5px 7px;background:var(--surf);border:1px solid var(--rule);border-radius:2px">' +
    '<div style="font:600 7px monospace;text-transform:uppercase;letter-spacing:.06em;color:var(--t4);margin-bottom:2px">' +
    label +
    '</div>' +
    '<div style="font:400 10px monospace;color:var(--t1)">' +
    value +
    '</div>' +
    '</div>'
  );
