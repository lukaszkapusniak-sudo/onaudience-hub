...
export function getCoTags(c) {
  ...
  return TAG_RULES.filter((r) => r.kw.some((k) => hay.includes(k))).map((r) => r.tag);
}

export function getAv(n) {
  let h = 0;
...
export function ini(n) {
  if (!n) return '?';
  // Split on spaces OR capital letters (handles camelCase like onAudience → OA)
  const words = (n || '')
...
export function tClass(t) {
  return { client: 'tc', partner: 'tp', prospect: 'tpr', nogo: 'tn', poc: 'tpo' }[t] || 'tn';
}
export function tLabel(t) {
  return (
    { client: 'Client', partner: 'Partner', prospect: 'Prospect', nogo: 'No Outreach', poc: 'POC' }[
      t
    ] || t
  );
}
