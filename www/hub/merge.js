...
import { mergeSuggestions as dbMerge } from './db.js?v=20260410d22';
import S from './state.js?v=20260410d22';
import { esc, _slug, authHdr } from './utils.js?v=20260410d22';
import { clog } from './api.js?v=20260410d22';
...
export async function executeMerge(winnerId, loserId) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Merge failed ${res.status}: ${t.slice(0, 200)}`);
  }
...
export async function loadMergeSuggestionsCount() {
  try {
    const res = await fetch(`${SB_URL}/rest/v1/merge_suggestions?status=eq.pending&select=id`, {
...
window._pickMergeSource = function () {
  ...
  window._mergeSourceSearch = async function (q) {
    ...
    wrap.innerHTML = hits
      .map(
        (c) =>
          `<div class="mrg-result-row" onclick="window._selectMergeSource('${esc(c.id)}')">${esc(c.name)} <span style="color:var(--t3);font-size:10px">${esc(c.type || '')}${c.category ? ' · ' + esc(c.category) : ''}</span></div>`,
      )
      .join('');
  };
...
window._mergeSearch = async function (q) {
  ...
  wrap.innerHTML = filtered
    .map(
      (c) =>
        `<div class="mrg-result-row" onclick="window._selectMergeTarget('${esc(c.id)}')">${esc(c.name)} <span style="color:var(--t3);font-size:10px">${esc(c.type || '')}${c.category ? ' · ' + esc(c.category) : ''}</span></div>`,
    )
    .join('');
};
...
window._confirmMerge = async function () {
  ...
  try {
    ...
    const acts = document.getElementById('mergeActions');
    if (acts)
      acts.innerHTML += `<div style="color:var(--prc);font-size:11px;margin-top:4px">${esc(e.message)}</div>`;
    if (btn) {
      btn.disabled = false;
      btn.textContent = '⚙ Merge — keep TARGET name';
    }
  }
};
...
async function renderSuggestionsTab(body) {
  ...
  updateTabBtns();
  if (!items.length) {
    body.innerHTML = '<div class="mrg-empty">✓ No merge suggestions</div>';
    return;
  }
...
function showToast(msg) {
  ...
  if (!t) {
    ...
    t.id = 'mergeToast';
    t.style.cssText =
      'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:var(--surf2);border:1px solid var(--rule);border-radius:2px;padding:7px 16px;font:500 11px/1 "IBM Plex Mono",monospace;color:var(--t1);z-index:1100;box-shadow:var(--sh);transition:opacity .3s';
