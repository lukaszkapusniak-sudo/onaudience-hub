...
import { anthropicFetch, lemlistFetch } from './api.js?v=__OA_ASSET_VERSION__';
import { audiences as dbAud, companies as dbCo } from './db.js?v=__OA_ASSET_VERSION__';
...
export async function generateCampaignHook(audId, personaId) {
  ...
  const persona = personaId
    ? (window.MC_PERSONAS_LIST || []).find((p) => p.id === personaId)
    : null;
  ...
  const cos = (S.companies || [])
    .filter((c) => coIds.includes(c.id || c.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-')))
    .slice(0, 6);
...
export async function generateEmailTemplate(audId, personaId) {
  ...
  try {
    const res = await anthropicFetch({
      system: persona
        ? persona.system +
          '\n\nYou are writing B2B email templates for onAudience, which sells EU first-party audience data.'
        : undefined,
      model: MODEL_CREATIVE,
      max_tokens: 400,
      messages: [
...
export async function saveCampaignTemplate(audId) {
  ...
  try {
    await dbAud.patch(audId, {
      outreach_hook: hook,
      template_subject: subject,
      template_body: body,
    });
    if (aud) {
      aud.outreach_hook = hook;
      aud.template_subject = subject;
      aud.template_body = body;
    }
...
export async function audDraftEmailToCo(audId, coSlug) {
  ...
  const coContacts = audContacts.filter(
    (ct) => ct.company_id === co.id || _slug(ct.company_name || '') === _slug(co.name),
  );
  ...
  try {
    const res = await anthropicFetch({
      ...
      messages: [
        {
          role: 'user',
          content: `Write 1 short outreach angle for approaching ${co.name} (${co.category || co.type || ''}) about onAudience EU first-party audience data partnerships. Audience context: "${aud.name}". 1–2 sentences, very specific.`,
        },
      ],
    });
